...
export default function Example() {
  let [mapRef] = useState();
  let [currentPositionPointsArray] = useState([118.10388605, 24.48923061]);
  let [currentPositionPointsObj] = useState();
  useEffect(() => {
  ...
  const initMap = async () => {
    /*global BMap*/
    mapRef = new BMap.Map('bmap-hook-container')
    ...
    mapRef.setMapStyleV2({ styleJson: style_2 }) // 自定义地图
    // 浏览器获取当前定位
    try {
    ...
    // 生成点聚合
    addBMapMarkerClusterer(mapRef)
    // 添加工具条和比例尺
    controllerTools()
  }
  // 获取当前定位成功
  const getLocSuccess = curPosiObj => {
    const { lng, lat } = curPosiObj
    currentPositionPointsObj = new BMap.Point(lng, lat)
    mapRef.panTo(currentPositionPointsObj)
    // console.log('currentPositionPointsObj', currentPositionPointsObj)
  ...
  // 移除点标记
  const removeBMapMarker = marker => {
    if (marker) {
      // 移除指定点标记
      mapRef.removeOverlay(marker)
    }
  }
  // 添加点标记
  const addBMapMarker = (point, animation, iconConfig) => {
  ...
  const generateNearByNMarkers = (e, count = 10) => {
  ...
  // 添加信息窗口
  const bdMapAddInfoWindow = (windowConfig, marker, isCurrent) => {
    // console.log(marker)
  ...
  // 生成点聚合
  const addBMapMarkerClusterer = (mapRef) => {
    let markers = []
    let pt = null
    let i = 0
    for (; i < 200; i++) {
  ...
  return (
    <div className="app-container" style={{ position: 'relative' }}>
      <div id="bmap-hook-container"></div>
